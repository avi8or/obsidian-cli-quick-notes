...
Previous: pr-04b-append (ffbec389ad6215a9aab006322e121deadde449c7)
This PR: pr-04d-init (6dd855a6ffb65c4a4f513fbf72b3f1d76b742f4d)
...
```
 ...
 84 files changed, 18435 insertions(+), 87 deletions(-)
```
...
@@ -54,6 +60,26 @@ For detailed help (including examples) for a specific command:
...
+
+- `obsidian-cli/preferences.json` (default vault name + per-vault settings)
...
@@ -147,7 +184,9 @@ obsidian-cli open "{note-name}" --vault "{vault-name}"
...
@@ -156,30 +195,136 @@ obsidian-cli daily
...
@@ -228,15 +373,32 @@ obsidian-cli print "{note-name}" --vault "{vault-name}"
...
@@ -265,6 +427,8 @@ When moving/renaming, `obsidian-cli` updates links inside your vault to match th
...
@@ -292,6 +456,9 @@ obsidian-cli delete "{note-path}"
...
+
 ...
 ```
@@ -303,6 +470,8 @@ Fork the project, add your feature or fix and submit a pull request. You can als
 ## Acknowledgements
...
```
git diff --no-color pr-04b-append..pr-04d-init
```
...
```
 ...
 vendor/gopkg.in/yaml.v2/yamlprivateh.go            |  173 ++
 vendor/modules.txt                                 |    9 +
 93 files changed, 19492 insertions(+), 122 deletions(-)
```
...
@@ -48,6 +54,32 @@ For full installation instructions, see [Mac and Linux manual](https://yakitrak.
 obsidian-cli --help
 ```
...
+
 ...
 The `search`, `search-content`, `create`, and `move` commands support the `--editor` (or `-e`) flag, which opens notes in your default text editor instead of the Obsidian application. This is useful for quick edits or when working in a terminal-only environment.
@@ -83,6 +115,26 @@ obsidian-cli set-default "{vault-name}"
...
+Open a note in Obsidian by vault-relative note path.
 ...
 ```bash
 # Opens note in obsidian vault
@@ -121,7 +184,9 @@ obsidian-cli open "{note-name}" --vault "{vault-name}"
...
+Note: creation/templates are controlled by Obsidian’s daily note settings/plugins. Use `append` (below) if you want the CLI to create/write daily notes itself.
 ...
 ```bash
 # Creates / opens daily note in obsidian vault
@@ -130,6 +195,134 @@ obsidian-cli daily
 # Creates / opens daily note in specified obsidian vault
 obsidian-cli daily --vault "{vault-name}"
...
+Created={{date:YYYY-MM-DD}} {{time:HH:mm}}
 ...
 ### Search Note
@@ -180,15 +373,32 @@ obsidian-cli print "{note-name}" --vault "{vault-name}"
...
+# Creates empty note in given obsidian
 ...
 # Creates note in default obsidian with content
@@ -210,7 +420,14 @@ obsidian-cli create "{note-name}" --content "abcde" --open --editor
...
+Note: `--editor` only applies when `--open` is also provided.
 ...
 ```bash
 # Renames a note in default obsidian
@@ -230,11 +447,19 @@ obsidian-cli move "{current-note-path}" "{new-note-path}" --open --editor
...
@@ -242,6 +467,12 @@ obsidian-cli delete "{note-path}" --vault "{vault-name}"
...
### Full Diff Command (includes vendor/)
```
git diff --no-color origin/main..pr-04d-init
```
